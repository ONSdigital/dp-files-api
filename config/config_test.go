	...
	Convey("Given an environment with no environment variables set", t, func() {
		Convey("When the config values are retrieved", func() {
			...
			Convey("Then the values should be set to the expected defaults", func() {
				...
				So(testCfg.MongoConfig.Collections, ShouldResemble, map[string]string{MetadataCollection: "metadata", CollectionsCollection: "collections", BundlesCollection: "bundles", FileEventsCollection: "file_events"})
				...
				So(testCfg.AuthConfig.Enabled, ShouldEqual, false)
				...
				So(testCfg.AuthConfig.PermissionsCacheUpdateInterval, ShouldEqual, time.Minute*1)
				So(testCfg.AuthConfig.PermissionsMaxCacheTime, ShouldEqual, time.Minute*5)
